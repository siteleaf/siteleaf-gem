module SiteleafCommands
  module Execution
    ...
    module Push
      ...
      def self.create_asset(updated_count, path, site_id)
      ...
      def self.file_valid?(path, ignore_paths)
      ...
      def self.assets_find(path, assets)
        assets.find { |asset| asset.filename == path }
      end

      def self.asset_valid?(asset, path)
        asset.nil? || (asset && asset.checksum != Digest::MD5.hexdigest(File.read(path)))
      end

      def self.upload_assets(updated_count, assets, ignore_paths, site_id)
      ...
      def self.get_missing_assets(assets)
        missing_assets = []
        assets.each { |asset| missing_assets << asset unless Dir.glob('**/*').include?(asset.filename) }
        missing_assets
      end

      def self.delete_missing_assets(missing_assets)
      ...
      def self.cleanup_old_assets(updated_count, assets)
      ...
      def self.put_theme_assets(site_id)
